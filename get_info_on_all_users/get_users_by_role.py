...
role_types_count = {}
allowed_roles = ['admin', 'read_only_user', 'read_only_limited_user', 'user', 'limited_user', 'observer',
                 'restricted_access', 'owner']
team_managers = []


def write_rows(column1, column2, column3, column4):
...
def get_users(role, session):
    ...
    for user in session.iter_all('users'):
        if user['role'] == role:
            ...
            write_rows(user['name'], role, user['email'], user['id'])
...
def get_managers(team_id, team_name, session):
    for member in session.iter_all('teams/%s/members' % team_id):
        if member['role'] == 'manager':
            ...
            write_rows(user['name'], "Team Manager, " + team_name, user['email'], user['id'])
